...
- **Tests**: Ginkgo + Gomega. Mocks for the `service` ports are generated by
  ...
  fails if the committed mocks are stale. Package-level specs live next to
  the code; specs that wire the full production graph through the DI
  container live in `test/integration`.
